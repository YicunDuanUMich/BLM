...
class FastReIDEstimator:
    ...
    def inference(self, image, bboxes):
        if bboxes is None or np.size(bboxes) == 0:
            return []
        ...
        batch_patches = []
        patches = []
        for d in range(np.size(bboxes, 0)):
            tlbr = bboxes[d, :4].astype(np.int_)
            tlbr[0] = max(0, tlbr[0])
            tlbr[1] = max(0, tlbr[1])
