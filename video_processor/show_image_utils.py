...
def resize_image(image, new_size=(-1, 400), interpolation=cv2.INTER_LINEAR):
    '''
    ...
    return cv2.resize(image, image_size_new, interpolation=interpolation)  # )


def show_image_with_resize(image_name, image, new_size=(-1, 400)):
    '''
    ...
    cv2.imshow(image_name, image_show)
    return image_show.shape
